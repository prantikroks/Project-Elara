/*
 * PROJECT ELARA: MASTER BLUEPRINT (FLUTTER FOUNDATION)
 * * This file is the architectural seed for Project Elara.
 * It establishes:
 * 1. The main app shell and 5-tab navigation.
 * 2. A "Luxury" theme (dark, premium, 'Inter' font).
 * 3. A scalable state management (Provider) for the focus timer.
 * 4. A prototype of the core Focus Screen with a "Neumorphic" button.
 *
 * --- PREREQUISITES ---
 * To run this, add these to your pubspec.yaml:
 * * dependencies:
 * flutter:
 * sdk: flutter
 * provider: ^6.0.5                # For scalable state management
 * google_fonts: ^6.1.0           # For the "Inter" font family
 * flutter_animate: ^4.5.0          # For "luxury" physics-based animations
 * percent_indicator: ^4.2.3      # For the circular timer
 * collection: ^1.18.0            # For ListEquality in TodoProvider
 * cached_network_image: ^3.3.1   # For "Headspace" style image loading
 * shimmer: ^3.0.0                # For the animated gradient title
 * rive: ^0.12.5                  # For the Pet Animation State Machine
 *
 * * flutter:
 * assets:
 * - assets/pet.riv          # Add your Rive animation file
 *
 * --- NEXT STEPS ---
 * 1. Build out each placeholder screen (Todo, Coach, Music, Account).
 * 2. Replace the "Pet Placeholder" with a Rive or Lottie animation.
 * 3. Begin the native integrations (AR, Health) using Platform Channels.
 */
...
import 'package:rive/rive.dart'; // For the Rive Pet Animation
...
void main() {
  ...
  runApp(
    MultiProvider(
      providers: [
        // The FocusTimerProvider now "leads"
        ChangeNotifierProvider(create: (context) => FocusTimerProvider()),
        ChangeNotifierProvider(create: (context) => TodoProvider()),
        ChangeNotifierProvider(create: (context) => UserProvider()),
        
        // --- THE PSYCHE-ACOUSTIC BRIDGE ---
        // The MusicProvider now *depends on* and *listens to* the FocusTimerProvider.
        // This is the "market-killer" connection.
        ChangeNotifierProxyProvider<FocusTimerProvider, MusicProvider>(
          create: (context) => MusicProvider(),
          update: (context, focusTimer, music) {
            if (music == null) return MusicProvider();
            // This is the "bridge"!
            // We are telling the MusicProvider about the user's stress state.
            music.updateFocusState(focusTimer.isStressed);
            return music;
          },
        ),
      ],
      child: const ProjectElaraApp(),
    ),
  );
}

class ProjectElaraApp extends StatelessWidget {
  const ProjectElaraApp({super.key});

  static const Color sereneBlue = Color(0xFF89CFF0);
  static const Color darkCharcoal = Color(0xFF2E2E2E);
  static const Color lightCharcoal = Color(0xFF3A3A3A);
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        // Use Google Fonts for the premium "Inter" font
...
// --- 2. STATE MANAGEMENT (THE "BRAIN" FOUNDATION) ---
...
class FocusTimerProvider with ChangeNotifier {
  ...
  FocusTimerProvider() : _currentSeconds = _totalSeconds {
...
class TodoProvider with ChangeNotifier {
  final List<Todo> _tasks = [
    ...
    Todo(id: '3', title: 'Architect "Empathetic Nudge"'),
...
class MusicProvider with ChangeNotifier {
  ...
  final List<MusicTrack> _playlist = [
    MusicTrack(
      ...
      gradientStart: ProjectElaraApp.sereneBlue.withOpacity(0.7),
      gradientEnd: ProjectElaraApp.lightCharcoal,
    ),
    MusicTrack(
      id: '2',
...
class UserProvider with ChangeNotifier {
  ...
  int _level = 40; // XP-Based
  int _xp = 350;
  int _xpForNextLevel = 1000;
  int _cumulativeHours = 100; // Time-Based
  ...
  List<Color> _getColorsForHours(int hours) {
    ...
    // Tier 1 (Cool Blue)
    return [ProjectElaraApp.sereneBlue, Colors.blue[800]!];
  }
}


// --- 3. MAIN APP SHELL (THE 5-TAB NAVIGATION) ---
...
// --- 4. CORE FOCUS SCREEN (THE "HEART" PROTOTYPE) ---
...
class FocusScreen extends StatelessWidget {
  ...
  // The name *must* match the one used in Swift & Kotlin.
        ...
        child: Column(
          ...
          children: [
            ...
            CircularPercentIndicator(
              ...
              progressColor: ProjectElaraApp.sereneBlue,
              backgroundColor: ProjectElaraApp.lightCharcoal,
            ...
            NeumorphicButton(
              onPressed: () {
                ...
                context.read<FocusTimerProvider>().startStopTimer();
              },
            ...
            IconButton(
              ...
              iconSize: 30,
              onPressed: () {
                context.read<FocusTimerProvider>().resetTimer();
              },
            ),
          ],
        ),
  ...
  // This maps your "Empathetic Nudge" state to a placeholder icon.
  // IconData _getPetIcon(PetState state) {
  //   ... (This is now replaced by Rive) ...
  // }
}
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: ProjectElaraApp.darkCharcoal,
        ...
        child: Icon(
          widget.icon,
          size: 60,
          color: ProjectElaraApp.sereneBlue,
        ),
      ),
    );
  }
          ...
          body: CustomScrollView(
            slivers: [
              SliverAppBar(
                ...
                backgroundColor: ProjectElaraApp.darkCharcoal.withOpacity(0.8),
                elevation: 0,
              ),
              // A list of the tasks
              SliverList(
        ...
        child: Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              filled: true,
              fillColor: ProjectElaraApp.lightCharcoal.withOpacity(0.5),
              border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(15),
                borderSide: BorderSide.none,
              ),
              suffixIcon: IconButton(
                icon: const Icon(Icons.add, color: ProjectElaraApp.sereneBlue),
                onPressed: _submit,
              ),
            ),
          ),
        ...
        leading: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              color: isDone ? ProjectElaraApp.sereneBlue : Colors.transparent,
            ),
            child: isDone
...
class FocusQualityCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: ProjectElaraApp.lightCharcoal,
        borderRadius: BorderRadius.circular(20),
      ),
      child: Row(
        children: [
...
class ProactiveNudgeCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.lightbulb_outline, color: ProjectElaraApp.sereneBlue, size: 28),
...
class SubjectBarChart extends StatelessWidget {
  ...
  Color _getColorForTag(String tag) {
    if (tag == '#School') return ProjectElaraApp.sereneBlue;
              ...
              return Column(
                children: [
                  Container(
                    ...
                    height: 40,
                    decoration: BoxDecoration(
                      color: ProjectElaraApp.sereneBlue.withOpacity(data[index]),
                      borderRadius: BorderRadius.circular(8),
                    ),
                  ),
                  const SizedBox(height: 8),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                StatItem(value: '30', label: 'Day Streak'), // This is hardcoded, pull from provider
              ],
            ),
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return Container(
          decoration: BoxDecoration(
            color: isUnlocked
                ? ProjectElaraApp.lightCharcoal
                : ProjectElaraApp.lightCharcoal.withOpacity(0.5),
            borderRadius: BorderRadius.circular(20),
          ),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
